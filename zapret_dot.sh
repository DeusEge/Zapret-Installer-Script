...
blockcheck_scan_level="" # 1:quick | 2:standart | 3:force
only_blocked_sites="" # y | n
...
wait_until_network_ready() {
    ...
    nm-online -t "$TIMEOUT" > /dev/null 2>&1
        if [ $? -ne 0 ]; then
            echo "ERROR: Network is not ready after ${TIMEOUT} seconds."
            return 1
            exit 1
    fi

    # 2. Wait for internet connectivity (IP reachability)
    ...
    # 3. Wait for DNS resolution
    ELAPSED=0
    while ! getent hosts google.com > /dev/null 2>&1; do
        sleep "$INTERVAL"
        ELAPSED=$((ELAPSED + INTERVAL))

        if [ "$ELAPSED" -ge "$TIMEOUT" ]; then
            echo "ERROR: DNS resolution is not working after ${TIMEOUT} seconds."
            return 3
            exit 3
        fi
    done
...
EOF
    ...
    while true; do
        echo
        echo "Choose blockcheck scan level"
        echo "1 : quick - time < 1m - scan as fast as possible to reveal any working strategy"
        echo "2 : standart - 1m < time < 5m - do investigation what works on your DPI"
        echo "3 : force - 30m < time - scan maximum despite of result"
        read -p "your choice (default: 1) : " blockcheck_scan_level
        blockcheck_scan_level=${blockcheck_scan_level:-1} # default value y
        if [[ "$blockcheck_scan_level" == "1" || "$blockcheck_scan_level" == "2" || "$blockcheck_scan_level" == "3" ]]; then
            echo -e "\e[36mselected: $blockcheck_scan_level\e[0m"
            echo
            break
        else
            echo -e "\e[93mInvalid input. Try again.\e[0m"
    ...
    while true; do
        echo
        read -p "Do you want Zapret to work only on blocked sites? May cause issues on some systems (y/N) : " only_blocked_sites
        only_blocked_sites=${only_blocked_sites:-n} # default value n
        only_blocked_sites=${only_blocked_sites,,} # lowercase
        if [[ "$only_blocked_sites" == "y" || "$only_blocked_sites" == "n" ]]; then
            echo -e "\e[36mselected: $only_blocked_sites\e[0m"
            echo
            break
        else
            echo -e "\e[93mInvalid input. Try again.\e[0m"
...
EOF
    ...
    if [[ "$only_blocked_sites" == "y" ]]; then
        dpi_parameter="$dpi_parameter --hostlist-auto=/opt/zapret/ipset/zapret-hostlist-auto.txt" # for just running for banned sites
    fi
