...
# checks if file backed up 1=backed up 0=not backed up
is_file_backed_up() {
    local SRC_FILE="$1"
...
if [[ "$uninstall_dot" == "y" ]]; then
    ...
    out=$(lsattr -d -- "/etc/resolv.conf" 2>/dev/null)
    if [[ ${out%% *}:4:1 == "i" ]]; then
        immutable_flag="+i"
    else
        immutable_flag="-i"
...
if [[ "$systemd_resolved_required" == "y" ]]; then
    if [[ "$process_manager" == "systemd" ]] && systemctl list-unit-files systemd-resolved.service >/dev/null 2>&1; then
...
if [[ "$supported_packet_manager" == "n" ]]; then
    if [[ "$systemd_resolved_installed" == "n" && "$systemd_resolved_required" == "y" ]]; then
        error="y"
        echo -e "\e[31msystemd-resolved\e[0m"
    fi
