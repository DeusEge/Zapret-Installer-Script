...
if [[ "$install_or_uninstall" == "u" ]]; then
    ...
    while true; do
        ...
        uninstall_zapret=${uninstall_zapret:-"n"} # default value y
...
install_zapret=${install_zapret,,} # lowercase
    if [[ "$install_zapret" == "y" || "$install_zapret" == "n" ]]; then
        ...
        while [ -d "/opt/zapret" ]; do
            while [[ !("$uninstall_zapret" == "y" || "$uninstall_zapret" == "n") ]]; do
                echo
                read -p "/opt/zapret file is already exist. Wanna remove it? (Y/n) : " uninstall_zapret
                uninstall_zapret=${uninstall_zapret:-"y"} # default value y
                uninstall_zapret=${uninstall_zapret,,} # lowercase
            done
            if [[ "$uninstall_zapret" == "y" ]]; then
                /opt/zapret/uninstall_easy.sh
        ...
        while [ -d "$HOME/Downloads/zapret-v71.4" ]; do
            while [[ !("$uninstall_zapret" == "y" || "$uninstall_zapret" == "n") ]]; do
                echo
                read -p "$HOME/Downloads/zapret-v71.4 file is already exist. Wanna remove it? (Y/n) : " uninstall_zapret
                uninstall_zapret=${uninstall_zapret:-"y"} # default value y
                uninstall_zapret=${uninstall_zapret,,} # lowercase
            done
            if [[ "$uninstall_zapret" == "y" ]]; then
        ...
        while [ -f "$HOME/Downloads/zapret-v71.4.zip" ]; do
            while [[ !("$uninstall_zapret" == "y" || "$uninstall_zapret" == "n") ]]; do
                echo
                read -p "$HOME/Downloads/zapret-v71.4.zip file is already exist. Wanna remove it? (Y/n) : " uninstall_zapret
                uninstall_zapret=${uninstall_zapret:-"y"} # default value y
                uninstall_zapret=${uninstall_zapret,,} # lowercase
            done
            if [[ "$uninstall_zapret" == "y" ]]; then
...
setup_dot=${setup_dot,,} # lowercase
    if [[ "$setup_dot" == "y" || "$setup_dot" == "n" ]]; then
        ...
        if ! command -v systemctl &> /dev/null; then
            ...
            echo -e "\e[31myou have to install systemd manually\e[0m"
            exit 1
        fi
        # --
        break
...
if [[ "$install_zapret" == "y" ]]; then
    echo
    read -p "Enter a website banned in your country (default: roblox.com) : " banned_site
    banned_site=${banned_site:-roblox.com}
    echo "selected: $banned_site"

    if [[ "$banned_site" == "roblox.com" ]]; then
        echo
        echo '     _ ,+z@@@@@@@@@@@wwwwp_    _,wwwww@@@@@@@@@@@p*w_             '
        echo '     *{#M*?`___   `[?T"%*f      _"**%"""[`__   _-*%Np_            '
        echo '     _` _  __,[[[[''``` w_       _w ``,[[[[[[L__    `               '
        echo '        __zMF"--@@@M1@` ]N_      Jb_]MT--@@@F"*W_                  '
        echo '        _ `%MMMMMMMWWMW `         `_ /MmWMMMMMMMM" _              '
        echo '                                                                  '
        echo '                                                                  '
        echo '                                                   _              '
        echo '                                                __m               '
...
if [[ "$sed_installed" == "n" || "$inetutils_installed" == "n" || "$nftables_installed" == "n" || "$curl_installed" == "n" || "$unzip_installed" == "n" || "$bind_installed" == "n" ]]; then
    if command -v apt-get &> /dev/null; then
        ...
        echo -e "\e[32mpacket manager is not supported\e[0m"
        echo -e "\e[32myou have to install packets manually if needed\e[0m"
