...
systemd_resolved_installed=""
systemd_resolved_required=""
...
if [[ "$systemd_resolved_required" == "y" ]]; then
    if command -v stubby &> /dev/null; then
        ...
        echo -e "\e[32msystemd-resolved is installed\e[0m"
    else
        systemd_resolved_installed="n"
        echo -e "\e[31msystemd-resolved is not installed\e[0m"
    fi
fi
# --
...
EOF
    ...
    elif [[ "$process_manager" == "sysvdinit" ]]; then
    /var/tmp/zapret-v71.4/install_easy.sh <<EOF
y
y
y
